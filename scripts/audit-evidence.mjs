...
assert.ok(service.includes('DriveApp.getFolderById(EVIDENCE_FOLDER_ID)'), 'Apps Script does not save evidence to Drive');
assert.ok(service.includes('folder.createFile(blob)'), 'Apps Script does not create image files');
assert.ok(service.includes('delete data.evidenceAttachments'), 'Raw Base64 evidence remains in report data');
assert.ok(service.includes('data.evidenceLinks'), 'Teacher report does not receive Drive links');

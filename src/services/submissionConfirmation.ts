...
export const classifySubmissionServerPayload = (expectedSubmissionId: string, server: any): SubmissionResult => {
  ...
  if (!Number.isInteger(server.evidenceCount) || !Array.isArray(server.evidenceLinks)) return { state: 'pending', message: 'Entrega enviada, pendiente de confirmación' };
  return { state: 'confirmed', message: server.message || 'Entrega confirmada.', submissionId: server.submissionId, evidenceCount: server.evidenceCount, evidenceLinks: server.evidenceLinks };
};
